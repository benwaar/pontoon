...
func TestTableLifecycle(t *testing.T) {
	...
	if table.Status != TableWaiting {
		t.Fatalf("expected waiting, got %s", table.Status)
	}
	...
	if joined.Status != TablePlaying {
		t.Fatalf("expected playing after join, got %s", joined.Status)
	}
	snap := Snapshot(joined)
	if len(snap.PlayerCards) != 2 {
		t.Fatalf("expected 2 player cards, got %d", len(snap.PlayerCards))
	}

	// Perform zero or more hit actions until auto-finished or still playing then stick.
	if joined.Status == TablePlaying {
	...
	// If still playing after hit, stick.
	current, _ := mgr.Get(table.ID)
	if current.Status == TablePlaying {
