...
func (m *Manager) Action(id, playerID, move string) (*Table, error) {
    ...
    defer m.mu.Unlock()
    t, ok := m.tables[id]
    if !ok { return nil, errors.New("table not found") }
    if t.Status != TablePlaying { return nil, errors.New("table not playing") }
    if t.PlayerID != playerID { return nil, errors.New("wrong player") }
    if t.PlayerHand == nil { return nil, errors.New("hand not initialized") }
    switch move {
    case "hit":
        if len(t.Deck) == 0 { return nil, errors.New("deck empty") }
        c := t.Deck[0]
        t.Deck = t.Deck[1:]
        t.PlayerHand.Add(c)
...
func Snapshot(t *Table) State {
    ...
    return State{
        ...
        Status: t.Status,
        PlayerID: t.PlayerID,
        PlayerCards: func() []domain.Card { if t.PlayerHand!=nil { return t.PlayerHand.Cards }; return nil }(),
        PlayerStatus: ps,
