// Package main starts the game service HTTP server exposing table and health endpoints.
package main
...
func main() {
	...
	http.HandleFunc("/api/table/join", func(w http.ResponseWriter, r *http.Request) {
		...
		if tableID == "" || playerID == "" {
			http.Error(w, "missing id or player", http.StatusBadRequest)
			return
		}
	...
	http.HandleFunc("/api/table/", func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.HasPrefix(r.URL.Path, "/api/table/") {
			http.NotFound(w, r)
			return
		}
		...
		if len(parts) == 2 && parts[1] == "action" && r.Method == http.MethodPost {
			...
			if body.Move == "" || body.Player == "" {
				http.Error(w, "missing move or player", http.StatusBadRequest)
				return
			}
